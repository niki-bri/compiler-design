...
let io_test path args =
  ...
  let asm_str = X86.string_of_prog asm_ast in
  let _ = Driver.write_file dot_s_file asm_str in
  let _ = Platform.link (dot_s_file::["cinterop.c"; "stack_test.s"]) exec_file in
...
let io_tests =
  ...
  ; "llprograms/stack_test1.ll", [], "Stack test passed"
  ; "llprograms/stack_test2.ll", [], "Stack test passed"
  ]
