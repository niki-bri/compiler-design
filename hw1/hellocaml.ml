...
(* TERMINOLOGY: "Object" level vs. "Meta" level
  ...
  inflet op_pushq  (opl: operand list) (m:mach): unit = 
  (* operand list should only be the value to be pushed to the stack *)
  let op = List.hd opl in 
  op_movq [op; Ind2(Rsp)] m;
  op_subq [~$8; Reg(Rsp)];
ix syntax, syntactic sugar, etc.
*)
...
let rec string_of (e:exp) : string =
  let stint (i:int64) : string = Int64.to_string i in 
  match e with 
    | Var v -> v
    | Const c -> stint c
...
let rec lookup (x:string) (c:ctxt) : int64 =
  match c with 
    | [] -> raise Not_found
    | (a, d)::tl -> if x = a then d else lookup x tl 
...
let rec optimize (e:exp) : exp =
  match e with 
    ...
    | Add(e1, Neg(e2)) -> if e1 = e2 then Const 0L else Add(optimize e1, Neg(optimize e2))
    | Add(Neg(e1), e2) -> if e1 = e2 then Const 0L else Add(Neg(optimize e1), optimize e2)
    ...
    | Neg(Neg(Neg(e1))) -> optimize (Neg(optimize e1))
...
*)
let string_insn (i: insn) : string = 
  match i with 
    | IPushC c ->  "IPushC " ^ (Int64.to_string c) ^ "L; "
    | IPushV v ->  "IPushV \"" ^ v ^ "\"; "
    | IMul ->  "IMul; "
    | IAdd ->  "IAdd; "
    | INeg ->  "INeg; "

let prog_to_string (p:program) : string =
  let rec aux (p:program) =
    match p with 
      | [] -> ""
      | h::tl -> (string_insn h) ^ (aux tl)  
  in 
  "[" ^ (aux p) ^ "]"
...
let string_p3 : string = prog_to_string p3
let string_p2 : string = prog_to_string p2 
let ans_p3 : int64 = run ctxt2 p3 
