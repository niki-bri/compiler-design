...
let part3_tests : suite = [
  ...
  GradedTest ("Problem3-3", 5, [
    ...
    ("rev5", assert_eqf (fun () -> rev [1;2;3]) [3;2;1]);
  ]);
  ...
  GradedTest ("Problem3-5", 5, [
    ...
    ("insert7", assert_eqf (fun () -> insert 1 [0;1;2]) [0;1;2]);
  ]);
  ...
  GradedTest ("Problem3-6", 5, [
    ...
    ("union8", assert_eqf (fun () -> union [0;1;2;4] [-1;1;3]) [-1;0;1;2;3;4]);
  ]);
]
...
let part4_tests : suite = [
  ...
  GradedTest ("Problem5", 15, [
    ...
    ("compile2", assert_eqf (fun () -> compile (Mult(Const 0L, Var "x"))) ([IPushC 0L; IPushV "x"; IMul]));
    ("compile3", assert_eqf (fun () -> compile (Mult(Var "y", Mult(e2, Neg e2)))) ([IPushV "y"; IPushV "x"; IPushC 1L; IAdd; IPushV "x"; IPushC 1L; IAdd; INeg; IMul; IMul]));
    ("compile4", assert_eqf (fun () -> ans_p3) (-63L));
  ]);
]
